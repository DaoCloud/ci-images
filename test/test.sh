...
do
  ...
  docker run daocloud/ci-gcc:${version} gcc --version 2>&1 | grep ${version}
done

## python
for version in 2.6 2.7 3.1 3.2 3.3 3.4
do
  docker run daocloud/ci-python:${version} which python
  docker run daocloud/ci-python:${version} which pip
  docker run daocloud/ci-python:${version} python --version 2>&1 | grep ${version}
done
...
do
  ...
  docker run daocloud/ci-ruby:${version} ruby --version 2>&1 | grep ${version}
done
...
do
  ...
  docker run daocloud/ci-node:${version} node --version 2>&1 | grep ${version}
done
